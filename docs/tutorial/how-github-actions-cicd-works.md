...
在我们的微信小程序项目开发过程中，随着代码量的增长和团队协作的需要，手动执行测试和部署变得越来越繁琐且容易出错。GitHub Actions CI/CD 就像是给我们的项目配备了一个**24 小时不间断的质量卫士**，确保：
...
graph TB
    ...
    subgraph "CI Pipeline - 持续集成"
        ...
        E --> F[安装pnpm]
        F --> G[缓存依赖 pnpm cache]
        G --> H[安装项目依赖 pnpm install]
        H --> I[代码检查 pnpm lint]
        I --> J[运行单元测试 pnpm test:frontend:unit]
        J --> K[构建项目 pnpm build:frontend]
        K --> L[上传测试报告]
    end
...
- `retention-days: 7`：报告保存 7 天后自动删除
